...
inline void case_window_created(
    HWND window_handle, 
    WINDOW_PROCEDURE_DATA &window_procedure_data
) {
...
inline void case_window_destroyed(WINDOW_PROCEDURE_DATA &window_procedure_data) {
    window_procedure_data.window_count--;
...
inline void case_paint(HWND window_handle) {
    WINDOW* window = get_window(window_handle);
...
if (!window) return DefWindowProc(window_handle, message, word_parameter, long_parameter);
    ...
    switch (message) {
        case WM_CREATE:
            case_window_created(window_handle, window -> window_procedure_data);
            break;
